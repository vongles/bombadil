# @pastex file: btds_upgrade_kernel_v46.sh
# @pastex exec: bash %f
# @pastex usage: Upgrades pastex to V4.6 (Env-Aware + Auto-Exec)
...
echo -e "\033[1;34m>> [KERNEL] Initiating Pastex V4.6 Upgrade...\033[0m"
...
if [ -f "$TARGET_FILE" ]; then
    cp "$TARGET_FILE" "$BACKUP_DIR/pastex_old_v45"
    echo "   + Backup created at $BACKUP_DIR"
fi

# 2. INJECT CODE (V4.6 PAYLOAD)
cat << 'EOF' > "$TARGET_FILE"
#!/bin/bash
# ==============================================================================
# B.T.D.S. VELOCITY TOOL: pastex v4.6 (Polymorphic)
...
pastex() {
    ...
    log_info "Ingesting clipboard data..."
    if command -v termux-clipboard-get &> /dev/null; then termux-clipboard-get > "$TEMP_BUFFER"
        ...
        log_err "No clipboard tool found."
    ...
    # Bombadil Config Lookup (Requires yq)
    local source_dir=$(yq '.paths.scripts_dir' "$CONFIG_FILE" -r 2>/dev/null)
    local deploy_dir=$(yq '.paths.deploy_dir' "$CONFIG_FILE" -r 2>/dev/null)
    ...
    # A. Install System Deps (The New Feature)
    if [[ -n "$meta_deps" ]]; then
        if [[ -n "$PKG_CMD" ]]; then
...
# 3. FINALIZE
chmod +x "$TARGET_FILE"
# Ensure the symlink is fresh
ln -sf "$TARGET_FILE" "$HOME/.local/bin/btds/pastex"

# 4. GIT SNAPSHOT
cd "$HOME/btds_dots" || exit
if [ -d ".git" ]; then
    git add btds_scripts/pastex
    git commit -m "System Upgrade: Pastex V4.6 (Env-Aware)"
    echo -e "\033[1;32m>> [GIT] Version 4.6 committed to repository.\033[0m"
fi

echo -e "\033[1;32m>> [SUCCESS] Pastex V4.6 Active.\033[0m"
echo "   New Features:"
echo "   1. Auto-executes by default."
echo "   2. Supports '# @pastex noexec: true'"
echo "   3. Supports '# @pastex deps: pkg1 pkg2' (Auto-detects yay/pkg/apt)"
