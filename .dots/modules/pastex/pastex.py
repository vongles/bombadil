#!/usr/bin/env python3
# @file: pastex
# @usage: The B.T.D.S. Velocity Tool (Clipboard-to-Execution)
# @legio_review: L.VIII, L.I

import os
import sys
import subprocess
import shutil
import re
from pathlib import Path

def log(level, msg):
    colors = {"INFO": "\033[94m", "OK": "\033[92m", "WARN": "\033[93m", "ERR": "\033[91m"}
    print(f"{colors.get(level, '')}>> [{level}] {msg}\033[0m")

def process_chunk(chunk):
    lines = chunk.splitlines()
    meta = {}
    for line in lines[:20]:
        line = line.strip()
        if not line.startswith("# @pastex"): continue
        parts = line.split(":", 1)
        if len(parts) < 2: continue
    
        key = parts[0].replace("# @pastex", "").strip()
        val = parts[1].strip()
        meta[key] = val
    if "file" not in meta: return

    # B.T.D.S. Pathing Structure
    home = Path.home()
    repo_scripts = home / "btds_dots" / "btds_scripts"
    target_bin = home / ".local" / "bin" / "btds"
    repo_scripts.mkdir(parents=True, exist_ok=True)
    target_bin.mkdir(parents=True, exist_ok=True)

    filename = meta["file"]
    script_path = repo_scripts / filename
    link_path = target_bin / filename

    # Write Source File
    with open(script_path, "w") as f:
        if "shebang" in meta:
            f.write(f"#!{meta['shebang']}\n")
            for line in lines:
                if not line.strip().startswith("# @pastex"):
                    f.write(line + "\n")
        else:
            f.write(chunk)
            
    # Set Permissions
    script_path.chmod(0o755)
    
    # Symlink Logic (Bombadil Compliant)
    if link_path.exists() or link_path.is_symlink():
        link_path.unlink()
    link_path.symlink_to(script_path)
    log("OK", f"Deployed: {filename}")

    # Boot Sequence
    if "boot" in meta:
        log("INFO", f"Running boot for {filename}...")
        os.system(meta["boot"].replace("%f", str(link_path)))

    # Execution Logic
    if meta.get("noexec") != "true":
        log("INFO", f"Executing {filename}...")
        args = meta.get("args", "")
        cmd = meta.get("exec", str(link_path)).replace("%f", str(link_path))
        os.system(f"{cmd} {args}")

def main():
    try:
        if shutil.which("termux-clipboard-get"):
            content = subprocess.check_output("termux-clipboard-get", text=True)
        elif shutil.which("wl-paste"):
            content = subprocess.check_output("wl-paste", text=True)
        elif shutil.which("xclip"):
            content = subprocess.check_output(["xclip", "-o"], text=True)
        else:
            log("ERR", "No clipboard tool found.")
            sys.exit(1)
    except Exception as e:
        log("ERR", f"Clipboard error: {e}")
        sys.exit(1)

    # OBFUSCATED DELIMITER: Prevents V4.7 self-splitting bug
    p_header = "# " + "@pastex file:"
    chunks = re.split(f'(?={p_header})', content)
    valid_chunks = [c for c in chunks if p_header in c]
    
    log("INFO", f"Detected {len(valid_chunks)} file(s) in buffer.")
    for chunk in valid_chunks:
        process_chunk(chunk.strip())

if __name__ == "__main__":
    main()
